...
#define MAX_SOCKET_CONNECTION 1024
...
pthread_mutex_t mutex_server = PTHREAD_MUTEX_INITIALIZER, mutex_cancel= PTHREAD_MUTEX_INITIALIZER, mutex_login = PTHREAD_MUTEX_INITIALIZER;
...
int numofcar=0;
int fee=0,pos=0,lot=0,grid=0,id_num=-1;
int totalfee=0;
char tmp[100];
...
bool reserve(int fd, int curnum,int lot,int grid,int carnn){
	pthread_mutex_lock(&mutex_server);
	if(parking[lot-1][grid-1]==0){
		parking[lot-1][grid-1] = 1;
		fee +=30;
		database[numofcar].push_back(carnn);
		...
		result[numofcar].push_back(carnn);
		numofcar +=1;
		char res[140] = {0};
		sprintf(res, "%d,%d,%d", lot, grid, carnn);
		//myfile << res;
		//cout << res;
...
void pick_up(int fd, int curnum,int carnn){
	pthread_mutex_lock(&mutex_server);
	for(int i=0;i<numofcar;i++){
		if(carnn == database[i][0]){
			pos = i;
			break;
		}
	}
...
bool check_in(int fd, int curnum,int lot,int grid,int carnn){
	pthread_mutex_lock(&mutex_server);
	if(parking[lot-1][grid-1]==0){
		parking[lot-1][grid-1] = 1;
		fee +=40;
		...
		numofcar +=1;
		char chk[140] = {0};
		sprintf(chk, "%d,%d,%d", lot, grid, carnn);
		//myfile << chk;
		//cout << chk;
					...
					if(isreserve){
						...
						if(strncmp(menu,"show",4)==0){
							...
							if(0<aa && aa<4 && 0<bb && bb<9){
								lot = aa+1;
								grid = bb+1;
								re = reserve(fd, cur_num,aa,bb,carnn);
								if(!re){
									memset(rcv, '\0', 100);
							...
							if(0<aa && aa<4 && 0<bb && bb<9){
								bool ck;
								ck = check_in(fd, cur_num,aa,bb,carnn);
								if(!ck){
									memset(rcv, '\0', 100);
									memset(rcv_tmp, '\0', 100);
									memset(a, '\0', 10);
